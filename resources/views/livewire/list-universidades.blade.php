...
<div>
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page"><a class="text-facebook"
                                                               href="{{ route('universities') }}">{{ __('Universities') }}</a>
            </li>
        </ol>
    </nav>
    <div class="row mb-4">
            ...
            <div class="search-form">
                <div class="input-group border border-primary rounded-sm">
                    ...
                    <input wire:model.live="search" class="form-control" type="text"
                           placeholder="{{ __('Search...') }}" autofocus autocomplete="search-universities">
                </div>
            </div>
        </div>
        @foreach ($modulePermissions as $modulePermission)
            @if($modulePermission->route_name === 'create_university')
                <div class="col-lg-6 col-md-4 col-sm-5 d-flex flex-column justify-content-center align-items-end">
                    <button wire:click="$dispatch('open-create-university-modal')"
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    @if (empty($search) && $listUniversities->count() == 0)
                    ...
                    @elseif ($listUniversities->count())
                        <div class="row">
                            <div class="col">
                                <p class="mb-3">
                                    @if($totalEntries > 0)
                                        {{ __('Showing') }}
                                        @if(!$listUniversities->firstItem())
                                            {{ $listUniversities->firstItem() }}
                                        @else
                                            {{ __('of') }}
                                        @endif
                                        {{ $listUniversities->firstItem() }} {{ __('to') }} {{ $listUniversities->lastItem() }} {{ __('of') }} {{ $listUniversities->total() }} {{ __('entries') }}
                                    @elseif($listUniversities->total() >= 0)
                                        {{ __('Showing') }}
                                        @if(!$listUniversities->firstItem())
                                            {{ $listUniversities->firstItem() }}
                                        @else
                                        ...
                                        @endif
                                        {{ __('to') }}
                                        @if($listUniversities->lastItem())
                                            {{ $listUniversities->lastItem() }}
                                        @else
                                            0
                                        @endif
                                        {{ __('of') }} {{ $listUniversities->total() }} {{ __('entries') }}
                                        @if($totalEntries > 0)
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <tr>
                                    <th>
                                        <a wire:click.prevent="sortBy('nombre')" role="button"
                                           href="#">
                                            <i class="mdi mdi-sort ml-0"></i> {{ __('University')  }}
                                        </a>
                                    </th>
                                    <th>
                                    ...
                                    @foreach ($modulePermissions as $modulePermission)
                                        @if($modulePermission->route_name === 'edit_university')
                                            <th>
                                                {{ __('Edit')  }}
                                            </th>
                                        @endif
                                        @if($modulePermission->route_name === 'delete_university')
                                            <th>
                                                {{ __('Delete')  }}
                                            </th>
                                        @endif
                                ...
                                <tbody>
                                @foreach ($listUniversities as $university)
                                    <tr>
                                        <td>
                                            {{ Str::limit($university->nombre, 30) }}
                                        </td>
                                        <td>{{  date('d-m-Y', strtotime($university->created_at)) }}</td>
                                        <td>{{  date('d-m-Y', strtotime($university->updated_at)) }}</td>
                                        @foreach ($modulePermissions as $modulePermission)
                                            @if($modulePermission->route_name === 'edit_university')
                                                <td>
                                                    <button
                                                        wire:click="loadUniversity('{{ config('app.debug') ? $university->id : encrypt($university->id) }}')"
                                            ...
                                            @if($modulePermission->route_name === 'delete_university')
                                                <td>
                                                    <button
                                                        wire:click="$dispatch('delete-prompt-university', '{{ config('app.debug') ? $university->id : encrypt($university->id) }}')"
                                                        type="button"
    ...
    @foreach ($modulePermissions as $modulePermission)
        @if($modulePermission->route_name === 'create_university')
            <div wire:ignore.self class="modal fade" id="createUniversity" data-backdrop="static" tabindex="-1"
                 role="dialog"
                 aria-labelledby="createUniversityLabel"
                 aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="createUniversityLabel">{{ __('Create University') }}</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form wire:submit="createUniversity">
                            <div class="modal-body">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <label for="name" class="control-label">{{ __('University') }}</label>
        ...
        @endif
        @if($modulePermission->route_name === 'edit_university')
            <div wire:ignore.self class="modal fade" id="editUniversity" data-backdrop="static" tabindex="-1"
                 role="dialog"
                 aria-labelledby="editUniversityLabel"
                 aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="editUniversityLabel">{{ __('Update University') }}</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
...
<script>
    window.addEventListener('open-create-university-modal', event => {
        $('#createUniversity').modal('show');
    });

    window.addEventListener('close-create-university-modal', event => {
        $('#createUniversity').modal('hide');
    });

    window.addEventListener('open-edit-university-modal', event => {
        $('#editUniversity').modal('show');
    });

    window.addEventListener('close-edit-university-modal', event => {
        $('#editUniversity').modal('hide');
    });

    window.addEventListener('universityCreated', () => {
        Swal.fire({
            title: '{{ __('Created') }}',
    ...
    window.addEventListener('universityUpdated', () => {
        Swal.fire({
            title: '{{ __('Updated') }}',
    ...
    @this.on('delete-prompt-university', universityId => {
        Swal.fire({
            ...
            @this.dispatch('goOn-Delete-University', {universityId: universityId})
