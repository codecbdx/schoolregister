@section('title', __('Payment Concepts'))
...
<div>
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page"><a class="text-facebook"
                                                               href="{{ route('concepts') }}">{{ __('Payment Concepts') }}</a>
            </li>
        </ol>
    </nav>
    <div class="row mb-4">
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    @if (empty($search) && $listConcepts->count() == 0)
                    ...
                    @elseif ($listConcepts->count())
                        <div class="row">
                            <div class="col">
                                <p class="mb-3">
                                    @if($totalEntries > 0)
                                        {{ __('Showing') }}
                                        @if(!$listConcepts->firstItem())
                                            {{ $listConcepts->firstItem() }}
                                        @else
                                            {{ __('of') }}
                                        @endif
                                        {{ $listConcepts->firstItem() }} {{ __('to') }} {{ $listConcepts->lastItem() }} {{ __('of') }} {{ $listConcepts->total() }} {{ __('entries') }}
                                    @elseif($listConcepts->total() >= 0)
                                        {{ __('Showing') }}
                                        @if(!$listConcepts->firstItem())
                                            {{ $listConcepts->firstItem() }}
                                        @else
                                        ...
                                        @endif
                                        {{ __('to') }}
                                        @if($listConcepts->lastItem())
                                            {{ $listConcepts->lastItem() }}
                                        @else
                                            0
                                        @endif
                                        {{ __('of') }} {{ $listConcepts->total() }} {{ __('entries') }}
                                        @if($totalEntries > 0)
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <tr>
                                    ...
                                    @foreach ($modulePermissions as $modulePermission)
                                        @if($modulePermission->route_name === 'edit_concept')
                                            <th>
                                                {{ __('Edit')  }}
                                            </th>
                                        @endif
                                        @if($modulePermission->route_name === 'delete_concept')
                                            <th>
                                                {{ __('Delete')  }}
                                            </th>
                                        @endif
                                ...
                                <tbody>
                                @foreach ($listConcepts as $concept)
                                    <tr>
                                        <td>
                                            {{ Str::limit($concept->nombre, 30) }}
                                        </td>
                                        <td>{{  date('d-m-Y', strtotime($concept->created_at)) }}</td>
                                        <td>{{  date('d-m-Y', strtotime($concept->updated_at)) }}</td>
                                        @foreach ($modulePermissions as $modulePermission)
                                            @if($modulePermission->route_name === 'edit_concept')
                                                <td>
                                                    <button
                                                        wire:click="loadConcept('{{ config('app.debug') ? $concept->id : encrypt($concept->id) }}')"
                                            ...
                                            @if($modulePermission->route_name === 'delete_concept')
                                                <td>
                                                    <button
                                                        wire:click="$dispatch('delete-prompt-concept', '{{ config('app.debug') ? $concept->id : encrypt($concept->id) }}')"
                                                        type="button"
    ...
    @foreach ($modulePermissions as $modulePermission)
        @if($modulePermission->route_name === 'create_concept')
            <div wire:ignore.self class="modal fade" id="createConcept" data-backdrop="static" tabindex="-1"
                 role="dialog"
                 aria-labelledby="createConceptLabel"
                 aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="createConceptLabel">{{ __('Create Payment Concept') }}</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form wire:submit="createConcept">
        ...
        @endif
        @if($modulePermission->route_name === 'edit_concept')
            <div wire:ignore.self class="modal fade" id="editConcept" data-backdrop="static" tabindex="-1" role="dialog"
                 aria-labelledby="editConceptLabel"
                 aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
...
<script>
    window.addEventListener('open-create-concept-modal', event => {
        $('#createConcept').modal('show');
    });

    window.addEventListener('close-create-concept-modal', event => {
        $('#createConcept').modal('hide');
    });

    window.addEventListener('open-edit-concept-modal', event => {
        $('#editConcept').modal('show');
    });

    window.addEventListener('close-edit-concept-modal', event => {
        $('#editConcept').modal('hide');
    });

    window.addEventListener('conceptCreated', () => {
        Swal.fire({
            title: '{{ __('Created') }}',
    ...
    window.addEventListener('conceptUpdated', () => {
        Swal.fire({
            title: '{{ __('Updated') }}',
    ...
    @this.on('delete-prompt-concept', conceptId => {
        Swal.fire({
            ...
            @this.dispatch('goOn-Delete-Concept', {conceptId: conceptId})
